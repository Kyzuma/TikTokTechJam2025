...
def check_transactions():
    ...
    ---
    get:
      summary: Run fraud detection on transactions (past 24hrs)
      description: |
    ...
    flagged = supabase.table("flagged_transaction") \
        .select("*") \
        .gte("created_at", since.isoformat()) \
        .execute().data
    ...
    for tx in transactions:
        ...
        if tx["amount"] > (limit * 10) or tx["amount"] > 1_000_000:  # arbitrary ridiculous threshold
            new_flags.append({
    ...
    pending_73 = [tx for tx in transactions if tx["from_user_id"] == 7 and tx["to_user_id"] == 3 and tx["status"] == "pending"]
    if len(pending_73) >= 5:
...
def recalc_trust_scores():
    ...
    ---
    post:
      summary: Recalculate trust scores for all users
      description: |
    ...
    for user in users:
        ...
        last_login = datetime.datetime.fromisoformat(user["last_login"].replace("Z", "+00:00"))
        created_at = datetime.datetime.fromisoformat(user["created_at"].replace("Z", "+00:00"))

        logs = supabase.table("trust_logs").select("*").eq("user_id", user_id).order("created_at", desc=True).limit(1).execute().data
...
def verify_user():
    ...
    ---
    get:
      summary: Verify a user account and update trust score
      description: |
    ...
    # 1. Fetch user
    user = supabase.table("user_profiles").select("*").eq("user_id", user_id).execute().data
    if not user:
